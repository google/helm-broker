...
func (c *k8sController) CreateServiceInstance(instanceID string, req *sbmodel.CreateServiceInstanceRequest) (*sbmodel.CreateServiceInstanceResponse, error) {
...
func (c *k8sController) getType(serviceID string, planID string) (*sbmodel.Types, error) {
...
func getTypesFromPlan(p *sbmodel.ServicePlan) (*sbmodel.Types, error) {
	err := fmt.Errorf("Did not find usable types for plan %s", p.ID)

	if p.Metadata == nil {
		return nil, err
	}
	if _, ok := p.Metadata.(map[string]interface{})[sbmodel.InstanceType]; !ok {
		return nil, err
	}
	if _, ok := p.Metadata.(map[string]interface{})[sbmodel.BindingType]; !ok {
		// No binding type... cool, just return the instance type
		return &sbmodel.Types{
			Instance: p.Metadata.(map[string]interface{})[sbmodel.InstanceType].(string),
		}, nil
	}
	return &sbmodel.Types{
		Instance: p.Metadata.(map[string]interface{})[sbmodel.InstanceType].(string),
		Binding:  p.Metadata.(map[string]interface{})[sbmodel.BindingType].(string),
	}, nil
}

func (c *k8sController) getSchemas(t sbmodel.Types) (*sbmodel.Schemas, error) {
